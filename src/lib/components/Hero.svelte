...
<section id="home" class="min-h-screen flex items-center pt-16 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-background to-muted/30">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
      <FadeIn delay={200}>
        <div class="flex flex-col space-y-6 max-w-xl">
          ...
          <p class="text-xl sm:text-2xl font-medium text-muted-foreground">{title}</p>
      ...
      <FadeIn delay={400}>
        <div class="relative aspect-square w-full max-w-md mx-auto lg:ml-auto">
          <div class="absolute inset-0 bg-gradient-to-br from-primary/20 to-primary/5 rounded-full"></div>
          <img 
            src={photoUrl} 
            alt={fullName} 
          ...
          <div class="absolute -top-4 -right-4 w-20 h-20 border-2 border-primary/30 rounded-full animate-pulse"></div>
          <div class="absolute -bottom-6 -left-6 w-24 h-24 border-2 border-primary/20 rounded-full animate-pulse" style="animation-delay: 1s"></div>
        </div>
      </FadeIn>
    </div>
  </div>
